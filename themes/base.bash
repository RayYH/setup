...
###########################################################################
# Common SCM theme prompt related envs
...
SCM_THEME_CURRENT_USER_PREFIX=' ☺︎ '
...
function git_prompt_minimal_info() {
  ...
  _git_hide_status && return

  SCM_BRANCH="${SCM_THEME_BRANCH_PREFIX}\$(git_friendly_ref)"

  if [[ -n "$(_git_status | tail -n1)" ]]; then
    SCM_DIRTY=1
...
function git_prompt_vars() {
  if ${SCM_GIT_USE_GITSTATUS} && command_exists gitstatus_query && gitstatus_query && [[ "${VCS_STATUS_RESULT}" == "ok-sync" ]]; then # use faster gitstatus
    SCM_GIT_GITSTATUS_RAN=true # use this in git.bash and below to choose git status output
  else
    SCM_GIT_GITSTATUS_RAN=false
  fi

  if _git_branch &>/dev/null; then
    SCM_GIT_DETACHED="false"
    SCM_BRANCH="${SCM_THEME_BRANCH_PREFIX}\$(git_friendly_ref)$(_git_remote_info)"
  else
    SCM_GIT_DETACHED="true"

    local detached_prefix
    if _git_tag &>/dev/null; then
      detached_prefix=${SCM_THEME_TAG_PREFIX}
    else
      detached_prefix=${SCM_THEME_DETACHED_PREFIX}
    fi
    SCM_BRANCH="${detached_prefix}\$(git_friendly_ref)"
  fi
  ...
  if [[ "${SCM_GIT_GITSTATUS_RAN}" == "true" ]]; then
    ...
    IFS=$'\t' read -r commits_behind commits_ahead <<<"$(_git_upstream_behind_ahead)"
  fi
  ...
  SCM_CHANGE=$(_git_short_sha 2>/dev/null || echo "")
}
...
function git_user_info() {
  ...
  [[ -n "${SCM_CURRENT_USER}" ]] && printf "%s" "$SCM_THEME_CURRENT_USER_PREFIX$SCM_CURRENT_USER$SCM_THEME_CURRENT_USER_SUFFIX"
}
