...
function _git_prompt_info() {
    ...
    if [[ "$(__git_prompt_git config --get setup.hideStatus 2>/dev/null)" != "1" ]]; then
        ...
        echo "$SET_UP_THEME_GIT_PROMPT_PREFIX${ref#refs/heads/}$(_parse_git_dirty)$SET_UP_THEME_GIT_PROMPT_SUFFIX"
    fi
}
...
function _git_prompt_short_sha() {
    local SHA
    SHA=$(_git_short_sha) && echo "$SET_UP_THEME_GIT_PROMPT_SHA_BEFORE$SHA$SET_UP_THEME_GIT_PROMPT_SHA_AFTER"
}

# Formats themes string for current git commit long SHA
function _git_prompt_long_sha() {
    local SHA
    SHA=$(_git_long_sha) && echo "$SET_UP_THEME_GIT_PROMPT_SHA_BEFORE$SHA$SET_UP_THEME_GIT_PROMPT_SHA_AFTER"
}
